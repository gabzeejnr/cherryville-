...
const PROGRAMS = [
  {
    ...
    description: "Transform raw data into powerful insights that drive business decisions.",
    duration: "12 Weeks",
  ...
  },
  {
    ...
    description: "Master the art and science of bringing successful products to market.",
    duration: "10 Weeks",
  ...
  },
  {
    ...
    description: "Understand AI and machine learning without writing a single line of code.",
    duration: "8 Weeks",
  ...
  },
  {
    ...
    description: "Become the vital link between business needs and technical solutions.",
    duration: "12 Weeks",
    commitment: "10-15 hours/week",
...
function ProgramCard({ program, onViewDetails }) {
  return (
    <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-md bg-white flex flex-col">
      ...
      <div className="p-6 flex flex-col flex-1 gap-4">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="text-lg font-bold text-slate-900 leading-snug mb-0.5">{program.title}</h3>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-100 mt-auto">
          <span className="text-xs text-gray-400 font-medium">Contact for pricing</span>
          <button
            onClick={() => onViewDetails(program)}
            className="inline-flex items-center gap-1 text-sm font-semibold transition-all hover:gap-2"
            style={{ color: program.iconBg }}
          >
            View Details
...
function ProgramModal({ program, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4" onClick={onClose}>
      <div className="w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-2xl bg-white shadow-2xl relative" onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} className="absolute top-4 right-4 w-8 h-8 rounded-full flex items-center justify-center text-gray-400 hover:bg-gray-100 hover:text-gray-700 transition z-10" aria-label="Close">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
            <path d="M6 6l12 12M18 6L6 18" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        </button>
        <div className="px-8 pt-8 pb-2">
          <div className="w-14 h-14 rounded-xl flex items-center justify-center mb-5" style={{ backgroundColor: program.iconBg }}>
            {program.icon}
          </div>
          ...
          <div className="flex items-center gap-3 mt-4 mb-2">
            ...
            <span className="inline-flex items-center gap-1.5 text-xs font-medium text-gray-600 bg-gray-100 px-3 py-1.5 rounded-full">{program.commitment}</span>
          </div>
        </div>
          ...
          <ol className="space-y-2">
            {program.curriculum.map((item, i) => (
              <li key={item} className="flex items-start gap-3 text-sm text-gray-700">
                <span className="w-5 h-5 rounded-full bg-gray-100 text-gray-500 text-xs font-semibold flex items-center justify-center shrink-0 mt-0.5">{i + 1}</span>
                {item}
              </li>
            ))}
          </ol>
        ...
        <div className="px-8 pb-8 pt-2 border-t border-gray-100 flex items-center justify-end">
          <a href={whatsappUrl(program)} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-[#25D366] hover:bg-[#20bd5a] text-white font-semibold text-sm transition">
...
export default function ProgramsSection() {
  ...
  return (
    <section id="programs" className="w-full min-h-screen py-20 px-8 lg:px-16 bg-white flex flex-col justify-center">
      ...
      <div className="grid sm:grid-cols-2 gap-6 mb-12 w-full max-w-4xl mx-auto">
