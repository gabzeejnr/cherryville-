...
function ServiceCard({ title, text }: ServiceLines) {
    return (
        <div className="flex flex-col gap-5 shadow-md hover:shadow-lg p-4 py-6 rounded-2xl overflow-hidden">
    ...
    return (
        <section className="px-3 md:px-5 py-20 bg-cherry">
            ...
            <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
                {serviceLines.map(service => <ServiceCard title={service.title} text={service.text} />)}
            </div>
        </section>
    )
}